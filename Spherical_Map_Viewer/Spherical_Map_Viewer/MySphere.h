...
GLuint MySphere::Init(int la_slice, int lo_slice, vec4 color)
{
	...
	for (int i = 0; i<la_slice; i++)
	{
		...
		for (int j = 0; j<lo_slice; j++)
		{
			...
			vec2 ta(1 - 1*j / float(lo_slice), 1 - 1*i / float(la_slice));
			vec2 tb(1 - 1*j / float(lo_slice), 1 - 1*(i + 1) / float(la_slice));
			vec2 tc(1 - 1*(j + 1) / float(lo_slice), 1 - 1*(i + 1) / float(la_slice));
			vec2 td(1 - 1*(j + 1) / float(lo_slice), 1 - 1*(i) / float(la_slice));
